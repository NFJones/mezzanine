...
#[derive(Debug, Clone)]
pub(in crate::runtime) struct RuntimeSessionComponent {
    session: Session,
...
impl RuntimeSessionComponent {
    ...
    pub(in crate::runtime) fn window_created_at_unix_seconds(&self) -> &BTreeMap<String, u64> {
        &self.window_created_at_unix_seconds
    }

    /// Returns window creation timestamps for transactional mutation.
    pub(in crate::runtime) fn window_created_at_unix_seconds_mut(
        &mut self,
    ...
    pub(in crate::runtime) fn replace_window_created_at_unix_seconds(
        &mut self,
    ...
    pub(in crate::runtime) fn lifecycle_state(&self) -> RuntimeLifecycleState {
        self.lifecycle_state
    }

    /// Replaces the application lifecycle state after a validated transition.
    pub(in crate::runtime) fn set_lifecycle_state(&mut self, state: RuntimeLifecycleState) {
        self.lifecycle_state = state;
    }

    /// Returns the canonical control socket path.
    pub(in crate::runtime) fn socket_path(&self) -> &Path {
        &self.socket_path
    }

    /// Returns the session creation timestamp.
    pub(in crate::runtime) fn created_at_unix_seconds(&self) -> u64 {
        self.created_at_unix_seconds
    }

    /// Returns the most recent primary-client attachment timestamp.
    pub(in crate::runtime) fn last_attach_at_unix_seconds(&self) -> Option<u64> {
        self.last_attach_at_unix_seconds
    }

    /// Records the most recent primary-client attachment timestamp.
    pub(in crate::runtime) fn set_last_attach_at_unix_seconds(&mut self, value: Option<u64>) {

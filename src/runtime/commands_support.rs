...
pub(super) fn runtime_mcp_add_command(
    service: &mut RuntimeSessionService,
    invocation: &CommandInvocation,
) -> Result<String> {
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(super) async fn runtime_mcp_add_command_async(
    service: &mut RuntimeSessionService,
    invocation: &CommandInvocation,
) -> Result<String> {
    let (server_id, transport, target, mutations) = runtime_mcp_add_mutations(invocation)?;
...
pub(super) fn runtime_mcp_remove_command(
    service: &mut RuntimeSessionService,
    invocation: &CommandInvocation,
) -> Result<String> {
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(super) async fn runtime_mcp_remove_command_async(
    service: &mut RuntimeSessionService,
    invocation: &CommandInvocation,
) -> Result<String> {
    let server_id = runtime_mcp_server_id(invocation, "mcp-remove requires a server id")?;
...
pub(super) fn runtime_mcp_retry_command(
    service: &mut RuntimeSessionService,
    invocation: &CommandInvocation,
) -> Result<String> {
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(super) async fn runtime_mcp_retry_command_async(
    service: &mut RuntimeSessionService,
    invocation: &CommandInvocation,
) -> Result<String> {
    let server_id = runtime_mcp_server_id(invocation, "mcp-retry requires a server id")?;
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
fn runtime_mcp_retry_display(report: &super::RuntimeMcpRetryReport) -> String {
    format!(
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(super) fn runtime_mcp_retry_event_payload(
    source: &str,
    report: &super::RuntimeMcpRetryReport,
) -> String {
    format!(
...
pub(super) fn runtime_apply_live_override_mutations(
    service: &mut RuntimeSessionService,
...
fn runtime_apply_permission_live_override(
    service: &mut RuntimeSessionService,
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(super) async fn runtime_apply_live_override_mutations_async(
    service: &mut RuntimeSessionService,
    mutations: Vec<ConfigMutation>,
) -> Result<(bool, bool, crate::runtime::RuntimeConfigApplyReport)> {
    let mut changed = false;
...
pub(super) fn runtime_mcp_server_status_for_display(
    service: &RuntimeSessionService,
    server_id: &str,
...
pub(super) fn runtime_mcp_server_id<'a>(
...
pub(super) fn runtime_mcp_transport_target(
    invocation: &CommandInvocation,
...
pub(super) fn runtime_repeated_flag_values(args: &[String], flag: &str) -> Vec<String> {
    args.windows(2)
...
pub(super) fn runtime_validate_config_identifier(value: &str, label: &str) -> Result<()> {
    if value.is_empty()
        || !value
            .bytes()
...
/// Runs the runtime list key bindings display operation for this subsystem.
///
/// The function keeps parsing, state changes, and error propagation in
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(super) fn runtime_bind_key_command(
    service: &mut RuntimeSessionService,
    invocation: &CommandInvocation,
) -> Result<String> {
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(super) fn runtime_unbind_key_command(
    service: &mut RuntimeSessionService,
    invocation: &CommandInvocation,
) -> Result<String> {
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(super) fn runtime_key_source<'a>(
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(super) fn runtime_default_prefix_bindings(escape: KeyChord) -> Vec<(KeyChord, &'static str)> {
    vec![
        (escape, "send-prefix"),

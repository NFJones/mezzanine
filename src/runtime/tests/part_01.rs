...
fn runtime_terminal_snapshot_commands_create_and_resume_snapshots() {
    ...
    assert!(create.contains(r#""body":"saved layout checkpoint""#), "{create}");
    ...
    assert!(resume.contains(r#""body":"loaded latest layout""#), "{resume}");
...
fn runtime_apply_patch_read_phase_truncation_dispatches_specific_error_plan() {
    ...
    assert!(
        !write_transaction.command.contains("read phase did not emit a snapshot"),
        "{}",
        write_transaction.command
    );
...
fn runtime_terminal_mcp_command_mutates_persisted_config_and_registry() {
    ...
    let output = service
        .execute_terminal_command(&primary, "mcp add fs --command mcp-fs --arg --root --arg . --disabled")
        .unwrap();

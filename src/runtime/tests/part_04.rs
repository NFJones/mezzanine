...
/// Verifies `/shell-mode native` drives a provider-visible shell command
/// through native execution and leaves the result visible to the continuation.
///
/// The integrated path should select native mode through the user command,
/// expose `shell_command` on the provider action surface, execute the command
/// without pane dispatch, record native transport metadata, and append the
/// successful action result to the next provider-turn context.
#[test]
fn runtime_shell_mode_native_provider_turn_executes_shell_command_and_continues() {
    let mut service = test_runtime_service();
    let primary = service
        .attach_primary("primary", true, Size::new(80, 24).unwrap(), 120)
        .unwrap();
    ...
    mark_test_pane_ready(&mut service, "%1");
    service.permission_policy_mut().set_approval_bypass(true);
    let cwd = temp_root("shell-mode-native-provider-turn");
    std::fs::create_dir_all(&cwd).unwrap();
    service
        .pane_current_working_directories
        .insert("%1".to_string(), cwd.clone());

    let mode = service.dispatch_runtime_control_body(
        r#"{"jsonrpc":"2.0","id":"shell-mode-native-e2e","method":"agent/shell/command","params":{"idempotency_key":"shell-mode-native-e2e","input":"/shell-mode native"}}"#,
        &primary,
    );
    assert!(mode.contains(r#""kind":"mutated""#), "{mode}");
    assert!(mode.contains("mode=native"), "{mode}");
    assert_eq!(
        service.agent_local_action_executor_for_pane("%1"),
        crate::runtime::RuntimeLocalActionExecutor::Native
    );

    let start = service.dispatch_runtime_control_body(
        r#"{"jsonrpc":"2.0","id":"native-provider-prompt","method":"agent/shell/command","params":{"idempotency_key":"native-provider-prompt","input":"create a native shell marker"}}"#,
        &primary,
    );
    assert!(start.contains(r#""state":"running""#), "{start}");
    service.pending_agent_provider_tasks.remove("turn-1");
    let provider = RuntimeBatchProvider {
        response: crate::agent::ModelResponse {
            provider: "runtime-batch".to_string(),
            model: "test".to_string(),
            raw_text: "native shell response".to_string(),
            usage: Default::default(),
            latest_request_usage: None,
            quota_usage: Default::default(),
            action_batch: Some(crate::agent::MaapBatch {
                protocol: "maap/1".to_string(),
                rationale: "test native shell mode e2e".to_string(),
                thought: None,
                turn_id: "turn-1".to_string(),
                agent_id: "agent-%1".to_string(),
                actions: vec![crate::agent::AgentAction {
                    id: "shell-1".to_string(),
                    rationale: "create native marker".to_string(),
                    payload: crate::agent::AgentActionPayload::ShellCommand {
                        summary: "Create native marker".to_string(),
                        command: "printf \"native-provider-output\\n\"; touch native-marker".to_string(),
                        interactive: false,
                        stateful: false,
                        timeout_ms: None,
                    },
    ...
    assert_eq!(execution.terminal_state, crate::agent::AgentTurnState::Running);
    assert!(
        execution
            .request
            .allowed_actions
    ...
    assert_eq!(result.status, crate::agent::ActionStatus::Succeeded);
    assert!(cwd.join("native-marker").exists());
    assert!(result.content_text().contains("native-provider-output"));
...
");
    assert!(
        context_text.contains("[action_result shell-1 shell_command succeeded]"),
        "{context_text}"
    );
    assert!(context_text.contains("execution_transport"), "{context_text}");
    assert!(context_text.contains("native"), "{context_text}");
    assert!(context_text.contains("sent_to_pane"), "{context_text}");
    assert!(context_text.contains("false"), "{context_text}");
    assert!(context_text.contains("native-provider-output"), "{context_text}");
    service.pane_processes_mut().terminate_all().unwrap();
}

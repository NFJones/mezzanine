...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
fn runtime_model_message_role_name(role: ModelMessageRole) -> &'static str {
    match role {
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
fn runtime_json_or_string(value: &str) -> serde_json::Value {
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
fn runtime_redacted_shell_view_text() -> &'static str {
...
fn runtime_bounded_trace_string_value(value: &str) -> serde_json::Value {
    serde_json::Value::String(runtime_bounded_trace_text(value))
}

/// Recursively bounds JSON string leaves before trace rendering.
fn runtime_bounded_trace_value_strings(value: serde_json::Value) -> serde_json::Value {
    match value {
...
fn runtime_trim_agent_pane_trace_log(log: &mut Vec<String>) {
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
fn runtime_sanitize_shell_view_value(
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
fn runtime_sanitize_action_result_context_text(
    content: &str,
    preserve_command_fields: bool,
) -> String {
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
fn runtime_sanitize_agent_diagnostic_text(value: &str) -> String {
...
fn runtime_maap_message_content_type(content_type: &str) -> String {
...
fn runtime_subagent_display_label(agent_id: &str, display_name: Option<&str>) -> String {
...
fn runtime_subagent_result_status_label(success: bool, summary: &str) -> &'static str {
    if success {
        "completed"
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
fn runtime_maap_action_payload_trace_json(
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
fn runtime_maap_action_trace_json(
    action: &AgentAction,
    preserve_command_fields: bool,
) -> serde_json::Value {
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
fn runtime_maap_batch_trace_json(
    batch: &crate::agent::MaapBatch,
    preserve_command_fields: bool,
) -> serde_json::Value {
    serde_json::json!({
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
fn runtime_model_request_trace_json(
...
fn runtime_openai_prompt_cache_diagnostics_trace_json(
    diagnostics: crate::agent::OpenAiPromptCacheDiagnostics,
) -> serde_json::Value {
    serde_json::json!({
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
fn runtime_model_response_trace_json(
...
fn runtime_agent_provider_error_trace_json(
    provider_id: &str,
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
fn runtime_action_result_trace_json(
    result: &ActionResult,
    include_shell_view: bool,
) -> serde_json::Value {
    serde_json::json!({
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
fn runtime_action_results_trace_json(

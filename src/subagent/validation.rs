...
impl SubagentScopeDeclaration {
    ...
    /// expansion.
    pub fn shell_command_violation(&self, command: &str) -> Result<Option<String>> {
    ...
    /// Runs the effect violation operation for this subsystem.
    ///
    /// The function keeps parsing, state changes, and error propagation in
    /// the owning module so callers receive typed results instead of relying
    /// on duplicated control-flow logic.
    fn effect_violation(&self, effects: &EffectiveCommandEffects) -> Option<String> {
        if effects.network
            || effects.credentials
        ...
        for path in &effects.reads {
            if !path_in_declared_scopes(&self.current_directory, path, &self.read_scopes) {
                return Some(format!(
        ...
        if effects.destructive && self.cooperation_mode == CooperationMode::ExploreOnly {

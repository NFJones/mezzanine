...
pub(in crate::terminal::render) fn render_styled_pane_lines(
    window: &Window,
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(in crate::terminal::render) fn render_pane_lines(
    window: &Window,
    pane: &mez_mux::layout::Pane,
    frame_context: &TerminalFrameContext,
    content: &[String],
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(in crate::terminal::render) fn render_pane_frame_template(
    window: &Window,
    pane: &mez_mux::layout::Pane,
    frame_context: &TerminalFrameContext,
    template: &str,
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(in crate::terminal::render) fn render_pane_frame_text(
    window: &Window,
    pane: &mez_mux::layout::Pane,
    frame_context: &TerminalFrameContext,
    template: &str,
...
pub(in crate::terminal::render) type PaneFrameRightStatusSegment = FrameStatusSegment<&'static str>;

/// Carries Pane Frame Right Value state for this subsystem.
///
/// The type keeps related data explicit so callers can inspect and move
/// structured runtime state without parsing display text.
pub(in crate::terminal::render) type PaneFrameRightValue = FrameStatusValue<&'static str>;

/// Carries Rendered Pane Frame Right Status state for this subsystem.
///
/// The type keeps related data explicit so callers can inspect and move
/// structured runtime state without parsing display text.
pub(in crate::terminal::render) type RenderedPaneFrameRightStatus =
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(in crate::terminal::render) fn pane_frame_row_layout(
    window: &Window,
    pane: &mez_mux::layout::Pane,
    frame_context: &TerminalFrameContext,
    template: &str,
...
pub(in crate::terminal::render) fn pane_frame_fill_char(template: &str) -> char {
...
pub(in crate::terminal::render) fn pane_frame_right_status(
    window: &Window,
    pane: &mez_mux::layout::Pane,
    frame_context: &TerminalFrameContext,
    template: &str,
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(in crate::terminal::render) fn pane_frame_right_aligned_values(
    window: &Window,
    pane: &mez_mux::layout::Pane,
    frame_context: &TerminalFrameContext,
    template: &str,
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(in crate::terminal::render) fn render_pane_frame_right_status(
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(in crate::terminal::render) fn pane_agent_shell_visible(
    frame_context: &TerminalFrameContext,
    pane_id: &str,
) -> bool {
    frame_context
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(in crate::terminal::render) fn pane_agent_prompt_space_reserved(
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(in crate::terminal::render) fn pane_agent_prompt_transparent(
    frame_context: &TerminalFrameContext,
    pane_id: &str,
) -> bool {
    frame_context
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(in crate::terminal::render) fn pane_frame_right_aligned_display_value(
    field: &str,
    value: String,
) -> String {
    if matches!(
...
pub(in crate::terminal::render) fn pane_frame_right_aligned_segment_value(
    field: &str,
    value: &str,
) -> String {
...
pub(in crate::terminal::render) fn compact_pane_working_directory(value: &str) -> String {

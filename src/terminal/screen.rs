...
/// Returns whether Mezzanine tracks save/restore state for one DEC private mode.
...
pub struct TerminalStyledLine {
    ...
    ///
    /// This lets presentation-only renderers show a transformed line while
    /// preserving the source text that should be written to paste buffers or
    /// host clipboards.
...
impl TerminalScreen {
    ...
    fn normal_reflow_source_rows(
        ...
        let last_visible_row = self
            ...
            .unwrap_or(self.cursor.row)
            .min(self.cells.len().saturating_sub(1));
        rows.extend((0..=last_visible_row).map(|row| PhysicalStyledLine {
            line: styled_line_from_row_with_copy_text(
                &self.cells[row],
                &self.renditions[row],
                self.line_copy_texts.get(row).cloned().flatten(),

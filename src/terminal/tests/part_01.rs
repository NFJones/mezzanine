...
use super::client_loop::{
    ...
    AttachedTerminalOutputWriteReport,
...
fn acid_grapefruit_builtin_theme_matches_documented_reference_palette() {
    let definition = builtin_ui_theme_definition("acid_grapefruit")
        .expect("missing acid_grapefruit theme");
    let expected_aliases = [
        ("primary", "#ff5f73"),
        ("secondary", "#d74f71"),
        ("tertiary", "#ff9a7a"),
...
fn acid_tangerine_builtin_theme_matches_documented_reference_palette() {
    let definition = builtin_ui_theme_definition("acid_tangerine")
        .expect("missing acid_tangerine theme");
    let expected_aliases = [
        ("primary", "#ffab3d"),
        ("secondary", "#d88a52"),
        ("tertiary", "#ffca8e"),
...
/// Returns whether a built-in theme intentionally preserves a full captured
/// user-configuration snapshot instead of the compact contrast-managed palette
/// invariants used by the derived built-ins.
...
impl AttachedTerminalClientLoopIo for FakeAttachedTerminalLoopIo {
    ...
    fn flush_pending_output(&mut self, max_bytes: usize) -> Result<AttachedTerminalOutputWriteReport> {
        if self.pending_output_bytes == 0 {
            return Ok(AttachedTerminalOutputWriteReport::completed(0));
        }

        let budget = self.write_budget_per_call.unwrap_or(max_bytes).min(max_bytes);

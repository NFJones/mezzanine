...
fn chat_completions_maap_tool_description(
    ...
    let routing_rule = "CRITICAL ROUTING RULE: When request_capability is in the allowed action types and an executable action type is needed but absent, request_capability is the ONLY correct response. Never emit a say action describing, diagnosing, or lamenting the absence of a capability. request_capability IS how you obtain the missing capability within the same turn; emit it immediately without any preceding say. If task-local facts such as repo owner/name, branch, commit, remotes, paths, or command forms can be derived from the checkout, request shell capability instead of asking the user.";
    let action_batch_envelope_rule = "The function call is the action-batch envelope, not a prerequisite step; do not emit a say-only or progress batch claiming that an initial or schema-valid batch is needed before the executable action. If an executable action is available and useful, put that action in this function call now.";
    match shim_kind {
...
fn deepseek_mcp_memory_routing_guidance(request: &ModelRequest) -> &'static str {
    ...
    "If this schema includes mcp_call, the MCP server and tool names are visible in the mcp_call variants; use them directly when the user names a matching server or the task matches visible MCP metadata. If runtime MCP context contains routing_match=available_mcp, treat that as current-turn evidence that mcp_call is a likely useful action in the same batch schema as other currently allowed actions. Do not use memory_search, memory_store, shell preflight, or request_capability for shell/network merely to set up a useful MCP call. If MCP arguments such as repo owner/name, branch, commit, or path can be derived from current action results, use those results directly; if they require local checkout inspection and shell is absent, request shell. Do not use memory_search to decide whether visible MCP metadata, action descriptions, or current action results are sufficient. If memory_search is present, use it only for a concrete durable prior-context gap; never use it to decide whether visible MCP metadata, action descriptions, or current action results are sufficient, and never emit duplicate memory_search actions in one batch."
}

...
fn openai_chat_completions_maap_tool(request: &ModelRequest) -> serde_json::Value {
    serde_json::json!({
        ...
        "function": {
            "name": OPENAI_MAAP_FUNCTION_TOOL_NAME,
            "description": format!(
                "Submit one validated Mezzanine MAAP action batch. Return a function call, not prose. The function call is the action-batch envelope, not a prerequisite step; do not emit a say-only or progress batch claiming that an initial or schema-valid batch is needed before the executable action. If an executable action is available and useful, put that action in this function call now. If this schema includes mcp_call and runtime MCP context contains routing_match=available_mcp, treat that as explicit current-turn evidence that mcp_call is the sane first action; do not choose memory_search, memory_store, shell preflight, or request_capability for shell/network first unless the user explicitly asks to recall or save persistent memory. Do not use a no-op memory action as a current-actions placeholder before MCP. {}",

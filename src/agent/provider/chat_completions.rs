...
pub struct ChatCompletionsProvider<T, D> {
    pub(in crate::agent) api_key: Option<SecretString>,
    pub(in crate::agent) provider_id: String,
    pub(in crate::agent) endpoint: String,
    pub(in crate::agent) stream: bool,
    pub(in crate::agent) timeout_ms: u64,
    pub(in crate::agent) transport: T,
    dialect: D,
}

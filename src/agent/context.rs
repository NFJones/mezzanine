...
/// Runs the append memory context operation for this subsystem.
///
/// The function keeps parsing, state changes, and error propagation in
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
...
/// Runs the append mcp context operation for this subsystem.
///
/// The function keeps parsing, state changes, and error propagation in
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
...
///
/// The surrounding contract makes repository instruction applicability,
/// precedence, and trust boundaries explicit every time the raw file content is
/// replayed. The raw content remains in the project-guidance context block so
/// it can be refreshed and compacted as one coherent unit.
...
/// Runs the append scheduler context operation for this subsystem.
///
/// The function keeps parsing, state changes, and error propagation in
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
...
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(super) fn insert_policy_context_block(blocks: &mut Vec<ContextBlock>, block: ContextBlock) {
...
/// Runs the block should precede policy operation for this subsystem.
///
/// The function keeps parsing, state changes, and error propagation in
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(super) fn block_should_precede_policy(source: ContextSourceKind) -> bool {
    matches!(
        source,
        ContextSourceKind::DeveloperInstruction
            | ContextSourceKind::Policy
...
/// Runs the block should precede project guidance operation for this subsystem.
///
/// The function keeps parsing, state changes, and error propagation in
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(super) fn block_should_precede_project_guidance(source: ContextSourceKind) -> bool {
    matches!(
        source,
        ContextSourceKind::DeveloperInstruction
            | ContextSourceKind::Policy
...
/// Runs the memory scope summary operation for this subsystem.
///
/// The function keeps parsing, state changes, and error propagation in
/// the owning module so callers receive typed results instead of relying
/// on duplicated control-flow logic.
pub(super) fn memory_scope_summary(scope: &MemoryScope) -> String {
    match scope {

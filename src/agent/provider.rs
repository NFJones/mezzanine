        ...
        [
            (
                ...
                serde_json::json!({
                    ...
                    "description": "Required terminal intent. Use progress only for nonterminal updates when the turn should continue and the message is useful independently of action logs. Progress is required for a checkpoint: already-observed progress, significant evidence that changes the working theory, an evidence-backed direction choice, concrete blockers, coherent phase transitions, validation results that determine the next step, or user-requested narration; do not use it for future-tense plans, intended-work checklists, or headings such as Plan:, Steps:, Next:, Executed:, or Evidence: when executable actions are requested in the same response. Do not use progress just to announce or justify executable actions in the same batch, but do include exactly one progress say when a checkpoint exists. Use final when the user goal is complete, and blocked when user input or an external condition is required before progress can continue. Do not pair final or blocked say actions with executable actions; wait for results first."
                }),
            ),
            (
                "content_type",
            ...
            (
                ...
                serde_json::json!({
                    "type": "string",
                    "minLength": 1,
                    "description": "Non-empty conversational text for the user. Content in say is display-only: shell commands and Mezzanine patch blocks here do not execute. Use shell_command when terminal work should be executed and apply_patch for executable *** Begin Patch blocks. Only include commands or patches here when the user explicitly asked to see examples or text. Do not use say to duplicate the batch rationale or action summaries. When using progress for a checkpoint, write 1-3 compact sentences naming the important fact, supported decision, and next direction. The text must state durable learning or a decision, not intended work. Do not format ordinary progress or final text with Plan:, Executed:, or Evidence: headings unless the user explicitly requested that report format. For markdown content, this remains the raw markdown copied to buffers and clipboards."
                }),
            ),
        ],

        ...
        [
            ...
            (
                ...
                serde_json::json!({
                    "type": "string",
                    "minLength": 1,
                    "description": "Non-empty conversational text for the user. Content in say is display-only: shell commands and Mezzanine patch blocks here do not execute. Use shell_command when terminal work should be executed and apply_patch for executable *** Begin Patch blocks. Only include commands or patches here when the user explicitly asked to see examples or text. Do not use say to duplicate the batch rationale, action summaries, recent thinking lines, or action results. When using progress for a substantial checkpoint, write 1-3 compact sentences naming the important fact, supported decision, and next direction. The text must state durable learning or a decision, not intended work. If there is no substantial new user-visible update, omit progress say. Do not format ordinary progress or final text with Plan:, Executed:, or Evidence: headings unless the user explicitly requested that report format. For markdown content, this remains the raw markdown copied to buffers and clipboards."
                }),
            ),
        ],

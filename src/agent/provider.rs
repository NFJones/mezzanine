        ...
        [
            (
                ...
                serde_json::json!({
                    ...
                    "description": "Required terminal intent. Use progress only for exceptional nonterminal updates when the turn should continue and the user would be materially confused without a visible update. Omit progress by default when executable actions are present. Include at most one progress say for a material change in diagnosis, implementation strategy, blocker state, validation outcome, or user-requested narration. Do not use progress for future-tense plans, intended-work checklists, routine inspection, owner localization, anchor lookup, test lookup, command-wrapper lookup, \"now patching\" updates, routine action continuity, or headings such as Plan:, Steps:, Next:, Executed:, or Evidence: when executable actions are requested in the same response. Do not use progress just to announce or justify executable actions, duplicate the batch rationale/action summaries, or restate prior progress in the same turn, and do not emit progress in every action batch. Use final when the user goal is complete, and blocked when user input or an external condition is required before progress can continue. Do not pair final or blocked say actions with executable actions; wait for results first."
                }),
            ),
            (
                "content_type",

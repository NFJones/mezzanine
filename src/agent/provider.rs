        ...
        [
            (
                ...
                serde_json::json!({
                    ...
                    "description": "Required terminal intent. Use progress only for nonterminal updates when the turn should continue and the message is useful independently of action logs. Progress is for substantial checkpoints: significant evidence that changes the working theory, an evidence-backed direction choice, concrete blockers, coherent phase transitions, validation results that determine the next step, or user-requested narration; do not use it for future-tense plans, intended-work checklists, routine action continuity, routine owner/test anchor lookup, command-wrapper lookup, or headings such as Plan:, Steps:, Next:, Executed:, or Evidence: when executable actions are requested in the same response. Do not use progress just to announce or justify executable actions, duplicate the batch rationale/action summaries, or restate prior progress in the same turn, and do not emit progress in every action batch. Use exactly one progress say when a substantial checkpoint exists. Use final when the user goal is complete, and blocked when user input or an external condition is required before progress can continue. Do not pair final or blocked say actions with executable actions; wait for results first."
                }),
            ),
            (
                "content_type",

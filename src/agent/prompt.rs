...
- mcp_call: call only MCP tools listed as available in the current runtime context, using the provided schema and tool identity. If the user task needs an external artifact or integration whose available MCP metadata matches the task, emit mcp_call as the first useful action. Do not emit a say-only setup batch claiming that a schema-valid or initial batch is needed before the MCP call; the current MAAP batch is the schema-valid wrapper for that call. Do not invent MCP tools or servers.\n\

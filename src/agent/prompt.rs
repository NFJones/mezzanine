...
/// Keeping this value documented makes the contract explicit at the module
/// boundary and avoids relying on call-site inference.
pub const AGENT_PROMPT_PROFILE_VERSION: u32 = 25;
...
- mcp_call: call only MCP tools listed as available in the current runtime context, using the provided schema and tool identity. If the user task needs an external artifact or integration whose available MCP metadata matches the task, emit mcp_call as the first useful action. When the user explicitly names an available MCP server or runtime context shows routing_match=available_mcp, the runtime may narrow this first action surface to MCP; do not request shell/network capability, run shell preflight, or use memory actions before trying the MCP route. Request another capability only after the MCP call result proves a concrete missing local fact or blocker. Do not emit a say-only setup batch claiming that a schema-valid or initial batch is needed before the MCP call; the current MAAP batch is the schema-valid wrapper for that call. Do not invent MCP tools or servers.\n\
...
pub(super) fn mcp_prompt(profile: &AgentPromptProfile) -> String {
    ...
    lines.push(
        "When the user names an MCP server and a matching exposed tool exists, prefer that server as the first concrete execution path and do not start with memory_search, memory_store, shell_command, request_capability for shell/network, or another indirect discovery action. If the runtime MCP integrations context contains routing_match=available_mcp, treat it as current-turn evidence that mcp_call is the sane next action; the runtime may narrow the first action surface to MCP so the integration is attempted before local preflight. Use memory_search or memory_store first only when the user explicitly asks to recall or save persistent memory. MCP-backed workflows normally forbid memory_search and memory_store unless the user explicitly asks to recall or save information, or a missing durable user preference is required and unavailable from current prompt or inspected artifacts. Do not use placeholder memory actions to satisfy a current-actions wrapper before the MCP call. If server purpose metadata is empty, match the request against the exposed tool descriptions; if no suitable tool exists, say so and use the best non-MCP path. Do not infer an MCP server's use case from its name alone; use the runtime MCP integrations manifest and active mcp_call schema as selection evidence. If the relevant server purpose or tool is unclear, ask for clarification or continue without MCP. After an MCP timeout, protocol error, or hang-like failure, do not loop on the same call without new evidence; fall back or report the blocker.".to_string(),
    );

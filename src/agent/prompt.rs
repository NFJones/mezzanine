...
- request_capability: non-executing controller routing for a missing action family. Use it immediately when shell, patch, web, MCP, config, messaging, or subagent capability is needed but absent. Use the relevant capability to safely gather missing task-local information when that information is needed for another action: shell for local workspace or process inspection, web/search/fetch for current external facts or explicit URLs, MCP for integration data, and config for Mezzanine configuration state. It is the correct response to a missing available-action family, and it takes precedence over blocked say, final say, or prose asking the user to enable/grant access. It is not a user permission request; do not ask the user to grant action capability.\n\

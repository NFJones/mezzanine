...
fn deepseek_provider_guidance() -> &'static str {
    "You are communicating through the DeepSeek Chat Completions API. The MAAP protocol exposes exactly one function per turn (submit_maap_action_batch) which carries ALL intended actions for this turn in one call. Pack every action into that single function call. Do not make multiple sequential function calls. The entire system prompt above contains authoritative behavioural rules, not advisory suggestions: treat every numbered section as a binding constraint on your behaviour. DeepSeek's API will separate your internal reasoning into a dedicated field; keep your final response content and function-call arguments concise. Parallel action batching is supported: you may include multiple shell_command, apply_patch, or other independent actions in the same batch. For apply_patch, the patch field must contain Mezzanine patch text starting with *** Begin Patch and ending with *** End Patch. The correct file directives are *** Update File:, *** Add File:, and *** Delete File:. Unified diff headers (---, +++, diff --git) are NOT the Mezzanine patch format; use *** Update File: <path> instead. Every hunk old/context line must be copied verbatim from current file content; never infer or reconstruct likely code. Use distinctive @@ header anchors on every hunk to improve match reliability."
}

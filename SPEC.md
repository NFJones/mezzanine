...
`agent_status_blocked_bg`, `agent_approval_attention_fg`,
`agent_approval_attention_bg`, `agent_status_failed_fg`,
...
logout, project trust decisions, permission profile display or
...
| `synchronize-panes` | Control synchronized primary input for the active window. It MUST accept `on`, `off`, `toggle`, and `status`; when enabled, ordinary primary pane input MUST be sent to every pane in the current window instead of only the active pane. |
...
process, recovery MUST request at most one fresh, instance-correlated foreground
observation for each blocked action at a time. It MUST count only consecutive
fresh observations of the same foreign foreground process group, reset that
stabilization when the primary process, shell-interaction generation, or foreign
process group changes, and settle the undispatched action only after its bounded
fresh-observation policy (three confirmations or a 1.5-second stabilization
deadline) is exhausted. Recovery MUST NOT inject shell input into
that process. A model-visible readiness or foreground-process dispatch failure
MUST include best-effort foreground process name, foreground and primary
process-group identifiers, the primary-shell-foreground determination, and
metadata availability/source; it MUST NOT include command lines, arguments,
environment values, or working directories. A managed routed child settled
...
the live viewport without closing the prompt. Agent prompt history MUST be bounded and shared across all agent sessions. It
MUST be loaded whenever an agent prompt is shown, including after `/resume`
binds a saved conversation into the pane. Loading or browsing shared prompt
history MUST NOT append pane output, transcript entries, model context, or
presentation-log entries.
...
Iroh endpoints MUST send QUIC keepalive probes at an interval below
...
`none`, default-level `zstd`, and LZ4. The checked-in command MUST report elapsed
time, throughput, allocation count and bytes, decoded bytes, and encoded wire
bytes. Host-dependent timings are report-only, but rollout review MUST preserve
these budgets: below-threshold frames bypass codec work; no codec may emit a
compressed representation larger than its input; compressible fixtures should
use at most 15 percent of decoded bytes for either compressed codec; and release
...
including any retained partial or deferred frame. Control-rendered views MUST
carry the painted presentation identities, and detached attach clients MUST
acknowledge them only after that complete local write. For Iroh render streams,
a successful server stream flush is the delivery approximation; identical
logical views carrying a new pending presentation identity MUST NOT be
suppressed, and pushed frames MUST NOT also use the control acknowledgement.
...
system prompt is present, the Anthropic Messages adapter MUST serialize the
...
usable signature MUST fail the waiting provider request closed with the
retained certification reason when one exists; the runtime MUST NOT fall back
to unresolved or unsandboxed execution. Repeated claims while bounded bootstrap
or certification work is pending MUST be idempotent.
...
OpenAI Responses cache identity MUST retain `reasoning`, `service_tier`, and
`text.verbosity`: these controls can affect provider cache compatibility even
when model-visible content is unchanged. Provider-native dialect spellings with
different verified semantics (for example DeepSeek `thinking` and Anthropic
`output_config.effort`), and sampling or output caps that providers do not fold
into cached prefix material (`temperature`, `stop`, `max_tokens`), MAY be
excluded from their provider-specific identity projections. Diagnostics MUST
make that provider-specific distinction explicit and MUST NOT claim that a
local projection proves provider cache continuity.
...
settles parent restoration. Settlement MUST release runtime input ownership so
subsequent user input reaches the foreign parent. After child certification, a
transient foreground process-group observation from a runtime-owned shell
action MUST NOT revoke the loader's restoration ownership; the live loader
...
MUST fail closed. When completion certification becomes pending, the runtime
MUST register its own bounded deadline independently of the pane worker's query
loop. The deadline MUST remain live after the completed bootstrap transaction
is removed. Expiry MUST discard the unpublished environment, retain a stable
certification rejection, clear the pane bootstrap gate, and resume or fail
...
event. The runtime MUST still enforce its independent completion-certification
...
Fresh processes created by layout restore MUST begin in `unknown` readiness and
MUST use this same event-driven prompt boundary. Restore MUST NOT infer prompt
readiness from process creation or arbitrary initial PTY activity, and MUST NOT
synchronously wait for startup output before returning.
...
A bootstrap attempt and every asynchronous certification phase that gates its
settlement MUST be bounded and one-shot for the observed pane readiness epoch.
Completing or failing a bootstrap wrapper, including certification timeout,
MUST clear the pending bootstrap dispatch for that pane. Mezzanine MUST NOT
repeatedly dispatch the same bootstrap wrapper on subsequent ticks solely
because the hidden bootstrap output could not be parsed.
...
the skill-specific semantic argument. Loaded skill bodies MUST use a distinct
skill context source rather than the source category used for the active user
prompt. The agent prompt selector SHOULD provide tab completion for
`$<skill-name>` from the effective skill catalog.
...
- `/show-memories`: Browse persistent memory records in the shared
  ...
  filtering list rows while highlighting matches, and list-row cursor movement
  MUST wrap at either boundary; Ctrl-Up and Ctrl-Down MUST move that cursor by
  five records. It MUST support browser-specific keys: `k` for kind dropdown selectors, `p` for
...
| `window/layout` | `{ "target": WindowTarget \| null, "layout": "tiled" \| "even-vertical" \| "even-horizontal" \| "even-grid", "idempotency_key": string }` | `{ "window_id": string, "layout": string, "state": LayoutState }` | Primary-only explicit layout selection. An omitted target selects the active window. The mutation MUST NOT change focus. |
| `window/rebalance` | `{ "target": WindowTarget \| null, "idempotency_key": string }` | `{ "window_id": string, "layout": string, "state": LayoutState }` | Primary-only reapplication of the target window's selected layout policy. An omitted target selects the active window. The mutation MUST NOT change focus. |
...
| `pane/rename` | `{ "target": PaneTarget \| null, "name": string, "idempotency_key": string }` | `{ "pane": PaneState }` | Primary-only explicit pane title assignment. The title is pinned against later process-title discovery. An omitted target selects the active pane. The mutation MUST NOT change focus. |
...
| `event/list` | `{ "after_event_id": integer \| null, "limit": integer \| null }` | `{ "events": [EventRecord], "latest_event_id": integer, "retained_from_event_id": integer \| null, "replay_retention": integer, "truncated": boolean }` | Read-only replay filtered to the caller's authorized audience. `limit` MUST NOT exceed the advertised replay-retention limit. |
...
For a resolved explicit invocation, the turn-local model context MUST identify the invoked server as referencable and direct the model to retrieve it with `mcp_server_get`; memory lookup or unrelated discovery is not a substitute. An explicit reference MUST NOT itself expose callable tools or authorize `mcp_call`. A successful retrieval MUST present every callable tool identity and description together with its complete validated input-schema contract, including required and optional fields, descriptions, nested structures, arrays, enums, and validation constraints, without silent truncation or omission. Mezzanine MUST preserve complete model-safe initialization instructions advertised by the MCP server as non-authoritative guidance, SHOULD derive a complete purpose from discovered tool metadata when operator-configured purpose is absent, and MUST preserve complete operator-configured purpose and usage guidance as higher-priority context. Credentials, transport configuration, environment variables, headers, approval internals, and other runtime-only metadata MUST remain excluded. If a provider context limit cannot accommodate the complete selected-server metadata, Mezzanine MUST fail the retrieval explicitly rather than present a partial manifest. For cache-stable provider schemas that use a generic `mcp_call` variant, post-retrieval turn-local context MUST instead identify callable server/tool pairs and their complete argument contracts, while controller and runtime validation MUST enforce those requirements before execution.
...
HTTP headers, bearer-token environment references, stored OAuth credentials, or
stored static bearer credentials created by `mez mcp login`. `mez mcp login
<id> --token <TOKEN>` MUST store the supplied static bearer token in the MCP
auth credential store rather than in `mcp_servers` configuration. MCP OAuth
login MUST use authorization-code PKCE. When no explicit client id is supplied
and authorization-server metadata advertises an RFC 7591
`registration_endpoint`, login MUST dynamically register a public native client
...
report stale credentials until the user logs in again. On stored-OAuth 401 or
403 responses, the runtime MUST attempt one refresh-and-retry when a refresh
...
MCP tool input schemas MUST be admitted through one lower-crate schema owner shared by snapshot restore, live discovery, and live tool-call planning, so admitted dialects, assertions, diagnostics, and compilations cannot drift between those paths. The supported dialects are JSON Schema 2020-12 (`https://json-schema.org/draft/2020-12/schema`), Draft 2019-09 (`https://json-schema.org/draft/2019-09/schema`), Draft-07 (`http://json-schema.org/draft-07/schema#`), Draft-06 (`http://json-schema.org/draft-06/schema#`), and Draft-04 (`http://json-schema.org/draft-04/schema#`). A schema MAY omit `$schema` (defaulting to 2020-12) or declare one supported dialect; the validator MUST use that dialect's native assertion semantics rather than silently reinterpreting it. Any other declared dialect, or a schema-position `$schema` that conflicts with the root selection, MUST be rejected as unsupported. Admitted assertions MUST include `type` (including type unions that admit objects), `enum`, `const`, `properties`, `patternProperties`, `additionalProperties`, `unevaluatedProperties`, `unevaluatedItems`, `required`, `dependentRequired`, `dependentSchemas`, `propertyNames`, `items`, `prefixItems`, `contains`, `minContains`, `maxContains`, `minItems`, `maxItems`, `uniqueItems`, `minProperties`, `maxProperties`, `minLength`, `maxLength`, `pattern`, `minimum`, `maximum`, `exclusiveMinimum`, `exclusiveMaximum`, `multipleOf`, `if`/`then`/`else`, `allOf`, `anyOf`, `oneOf`, `not`, and same-document reference applicators. `format` and the content keywords MUST remain annotations that never fail an otherwise valid call. `title`, `description`, `examples`, `default`, `$comment`, `deprecated`, `readOnly`, `writeOnly`, and extension keywords such as `x-*` MUST be treated as harmless metadata that never changes admission, assertion outcomes, approvals, or diagnostics. Keyword screening MUST descend only through schema positions (`properties`, `patternProperties`, `additionalProperties`, `items`, `prefixItems`, `contains`, `allOf`/`anyOf`/`oneOf`, `not`, `if`/`then`/`else`, `dependentSchemas`, `propertyNames`, `unevaluatedProperties`, `unevaluatedItems`, `$defs`, `definitions`, and schema-valued `dependencies`) and MUST treat the values of annotation and data keywords as data, so a reference- or dialect-shaped string inside them can neither withdraw a tool nor consume the reference budget.
Remote and file references MUST never be fetched, and validation MUST perform no network or filesystem access at admission or validation time. Only bounded same-document JSON Pointer `$ref` and `$dynamicRef` fragments MAY be admitted: the whole-document fragment `#` or a `/`-prefixed pointer fragment such as `#/$defs/kind`. Plain-name anchors such as `#name`, every other reference, and `$recursiveRef` MUST be rejected as unsupported without performing any I/O.
Schema admission, compilation, and validation MUST be explicitly bounded, including schema bytes, schema nesting depth, schema nodes (compilation work), reference count and per-reference bytes, tool-argument bytes, argument nesting depth, argument nodes (validation work), regular-expression backtracking and compiled size, and the number of compiled schemas retained per registry. Exceeding any bound MUST reject the schema or the call with a bounded diagnosis instead of unbounded work.
Schema and validation diagnostics MUST carry only a bounded category, a bounded canonical keyword, and a bounded instance pointer. They MUST NOT include instance values, schema text, credentials, or unbounded untrusted key text, and MUST be safe for model-visible context and audit records.
Compiled tool schemas MUST be cached under a stable generation derived from tool identity and exact schema bytes, MUST be evicted under a bounded capacity, and MUST be invalidated when MCP metadata refreshes so a pre-refresh compilation cannot answer a post-refresh plan. That generation MUST be a full SHA-256 digest over a length-prefixed encoding of the generation prefix, the server identity, the tool name, and the exact schema bytes, so distinct identities cannot alias and the digest MUST NOT be truncated for approval or dispatch identity.
MCP tool input schemas MUST be valid JSON object schemas and MUST describe object arguments when a root `type` is declared. Tools with malformed or unsupported argument schemas MUST remain non-callable, MUST NOT be represented as zero-argument tools, and SHOULD retain a bounded server/tool diagnostic for inspection. A rejected schema MUST withdraw only the affected tool: the owning server and its sibling tools MUST remain usable.
Live MCP tool-call planning MUST validate the call's arguments against the currently selected schema and MUST return the schema generation that validated them. That generation MUST be carried by the plan, the execution request, and the approval binding for the call. Live pre-transport revalidation MUST compare the currently selected schema generation with the recorded approval binding atomically with plan identity before any configured pre-MCP hook runs and before any transport is invoked. When the selected generation differs from the generation an approval was bound to, or when an approved call holds no bound generation because its approved arguments could not be re-planned against the then-selected schema, Mezzanine MUST settle only that unexecuted call with bounded no-dispatch evidence, MUST NOT run its pre-MCP hooks, MUST NOT invoke a transport for it, MUST NOT report it as successful, and MUST NOT replay the whole batch or re-run already successful siblings. MCP metadata refreshes MUST NOT create, widen, renew, or invent approval.
Model-authored arguments that fail a valid snapshot MUST surface as bounded repairable invalid-argument failures before batch dispatch, inside the existing bounded MAAP repair budget, and MUST NOT be repaired automatically beyond that budget. A malformed or unsupported server schema is a metadata or operator failure: the affected tool MUST become unavailable and MUST NOT be re-presented as a repairable model mistake. Ambiguous transport outcomes MUST NOT be retried, and authorization denial or cancellation MUST remain nonrecoverable. A settled failed, denied, or cancelled call MUST NOT authorize a later dispatch of the same action identity.
...
The parent agent-session directory MUST contain one bounded agent

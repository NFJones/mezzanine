...
| `terminal.resize_debounce_ms` | integer | `200` | Milliseconds to debounce terminal resize redraws and the single full redraw after a changed pane-divider drag is released. During a drag, Mez sends the acting client rate-limited divider-only updates while retaining pre-drag pane content; release restarts the debounce period. |

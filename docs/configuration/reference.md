...
| `agents.enabled_actions` | string array | all executable MAAP actions | Static action allowlist applied to every ordinary provider request. The provider schema always exposes the complete action catalog, while runtime validation rejects actions omitted here. Valid values are `say`, `shell_command`, `apply_patch`, `web_search`, `fetch_url`, `send_message`, `spawn_agent`, `config_change`, `mcp_server_search`, `mcp_server_get`, `mcp_call`, `memory_search`, `memory_store`, `issue_add`, `issue_update`, `issue_query`, and `issue_delete`; controller-only capability and skill actions are not configurable. Integration availability, permission, and argument checks still run when an enabled action is selected. |

...
| `terminal.resize_debounce_ms` | integer | `200` | Milliseconds to debounce terminal resize redraws and the single full redraw after a changed pane-divider drag is released. Divider movement still updates layout and PTY sizes immediately without rendering intermediate geometry; release restarts the debounce period. |

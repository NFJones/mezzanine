...
| `agents.peer_message_log_mode` | string | `"normal"` | Pane echo verbosity for peer MMP traffic. `normal` (default) logs model-authored `send_message` mail exactly as before and writes no echo row for runtime-owned subagent bridge notifications, because each one already has its own `subagent ...` status/result line. `verbose` logs the full bounded payload for every peer echo, bridge traffic and JSON traffic included, instead of the JSON `output` projection. Suppression is presentation-only: the durable peer-message block, delivery cursor, turn triggering, and approvals are unaffected. |

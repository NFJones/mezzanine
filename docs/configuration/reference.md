...
| `agents.shell_mode` | string | `"pane"` | Default agent shell execution transport: `pane` sends shell-backed actions through the pane shell; `native` runs each action in a freshly spawned shell inferred from the pane root process without sending pane input. Use `/shell-mode status` to view the effective pane mode, configured global mode, and override provenance in the pager. Use `/shell-mode pane` or `/shell-mode native` for an active-pane override, or append `--global` to persist the default for panes without an override. |

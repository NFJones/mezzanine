...
| `terminal.zen_mode` | boolean | `false` | Hide passive Mezzanine group, window, and pane bars and pills while retaining pane divider lines. This live, session-wide presentation setting preserves the underlying `frames.*` configuration so disabling zen mode restores the currently configured chrome. |
| `terminal.reduced_motion` | boolean | `false` | Disable optional frame/status animations. |
| `terminal.zen_focus_label_duration_ms` | integer | `1000` | Zen focus identity lifetime, 0–60000 milliseconds; zero disables and clears labels. Positive changes affect future labels only. Fixed pane top-left, window bottom-left and group top-left overlays preserve geometry and provider suppression. Records are primary-local; observers share the source deadline. |
...
| `terminal.resize_debounce_ms` | integer | `200` | Milliseconds to debounce terminal resize redraws and the single full redraw after a changed pane-divider drag is released. During a drag, Mez blanks the acting client's window body and sends rate-limited live-divider updates while retaining outer session chrome; release restarts the debounce period. |
| `terminal.render_rate_limit_fps` | integer | `30` | Maximum burst render frames per second for attached terminals and remote pushed-render streams; `0` disables render rate limiting. |
...
| `agent_transcript_peer_recipient_fg` | `"thinking"` | Recipient marker of an accepted outbound peer message from this pane. |
| `agent_transcript_peer_recipient_bg` | `"surface"` | Accepted outbound recipient marker background. |
...
| `agents.peer_message_log_mode` | string | `"normal"` | Pane echo verbosity for peer MMP traffic. In `normal` (default), canonical `text/plain; charset=utf-8` renders literally and supported `text/markdown` uses the safe Markdown renderer; JSON, binary, missing-media-type, and other payloads create no pane rows or presentation records. An accepted eligible model-authored send may create one `${recipient}<` row at its sender, while receivers create `{sender}>` rows only on commit; verbose mode logs bounded raw payloads for other accepted media, but runtime bridge traffic remains receiver-only. Sender rows prove acceptance or queueing only and never affect provider context, user trust, approvals, routing, turns, receipts, or cursors. Suppression is presentation-only, and settlement-time eligibility persists across replay. |
| `agents.session_title_policy` | string | `"generated"` | Source of the saved-session title shown in `/resume` rows and prompt completions. `generated` (default) sends one bounded side-channel request after the first prompt and then a bounded refresh every five inbound prompts (user or parent/peer), and uses the stored generated title, else the bounded objective, else the first prompt. Those requests always use the fast provider tier when supported, remain optional display work, and never block or fail a turn; `objective`, `last_prompt`, and `first_prompt` mirror that single source and never use the stored generated title. A manual `/name-session` name always wins, and `/name-session --clear` restores the derived title. Switching the policy away from `generated` is the opt-out and performs no provider call. |
...
`openai_prompt_cache_gpt56`, plus `openai_prompt_cache_explicit` and the
informational `vision` tag). The OpenAI
...
| `anthropic-fast` | `model` | `"claude-haiku-4-5"` |
...
| `permissions.env_whitelist` | string array | `["PATH", "HOME", "SHELL"]` when omitted | Portable names selected from Mez's immutable startup environment snapshot and forwarded to ordinary native and sandboxed actions. Values are bounded and redacted from status and logs; unlisted server variables are not forwarded. An explicit `[]` opts out. Internal semantic `apply_patch` phases retain their fixed environment. |
...
`/opt/acme-sdk` is mounted read-only at the same path. `ACME_HOME` must exist
when Mez starts, its value remains redacted from status and audit, and it grants
no filesystem authority. The default allowlist forwards `PATH` for
...
commands need the Mez-server command-search path. Changes made later by a pane
shell do not alter the captured values. Scope every required loader,

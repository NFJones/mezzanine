...
| `agents.peer_message_log_mode` | string | `"normal"` | Pane echo verbosity for peer MMP traffic. `normal` (default) logs every successfully committed received message once and every accepted outbound message once, including runtime-authored bridge traffic; JSON bridge status payloads without `output` render their bounded JSON. The only normal-mode suppression is the initial spawn status explicitly marked as duplicating the child pane's already-rendered parent prompt. `verbose` logs the full bounded payload for every peer echo instead of the JSON `output` projection. Suppression is presentation-only: the durable peer-message block, delivery cursor, turn triggering, and approvals are unaffected. |

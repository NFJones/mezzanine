...
| `agents.peer_message_log_mode` | string | `"normal"` | Pane echo verbosity for peer MMP traffic. Only a recipient's committed inbound message can create a pane row. `normal` (default) logs canonical `text/plain; charset=utf-8` literally and supported `text/markdown` through the safe Markdown renderer; JSON, binary, missing-media-type, and other payloads create no pane rows or presentation records. `verbose` logs the full bounded raw payload for every accepted media type at the receiving endpoint, including runtime bridge traffic. Suppression is presentation-only: durable peer-message blocks, delivery cursors, provider context, turn triggering, and approvals are unaffected. |

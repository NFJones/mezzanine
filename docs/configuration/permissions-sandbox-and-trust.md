...
The Bubblewrap backend requires a Linux runtime environment and is the default
Linux sandbox backend. Generated macOS configuration instead pairs
`auto-allow` approval with `policy-only` execution. Pane shell mode
...
Bubblewrap support use `policy-only` and therefore have no OS-level shell

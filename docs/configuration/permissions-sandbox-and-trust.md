...
The Bubblewrap backend requires a Linux runtime environment. When
...
with `policy-only`. Generated macOS configuration also pairs `auto-allow`
approval with `policy-only` execution. Pane shell mode

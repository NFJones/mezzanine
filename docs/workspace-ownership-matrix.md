...
| `src/agent/tests/` | split by behavior owner | temporary | Readiness, context appenders, request shaping, provider policy and intrinsic request/response behavior, semantic-patch planning, six execution-transcript projections, and seven shell-output transport decoder scenarios run directly in `mez-agent`. Root transcript coverage is one concrete-store persistence integration; root semantic and shell coverage exercises pane execution and generated wrappers; root provider coverage exercises auth, HTTP/process transport, quota, retry, and product error adapters. Move remaining intrinsic harness/MAAP tests and retain tests that exercise concrete product stores, transports, permissions, runtime, or UI. |

...
| `src/agent/context/` | `mez-agent` with root product-input adapters | temporary | Canonical records, deterministic compaction, context-to-message projection, source roles, evidence preservation, MCP prompt-summary recovery, cache identity, provider request defaults, and loaded-skill action constraints are lower-owned. Root request assembly now supplies only turn identity and embedded prompt assets; move dependency-neutral appenders and intrinsic tests while retaining concrete guidance discovery, memory stores, MCP registries, permissions, and runtime records behind narrow inputs. |

...
| `src/agent/context/` | `mez-agent` | temporary | Canonical capability, action-surface, context-source, model-message, complete `ModelRequest`, and model-profile policy records are imported directly from `mez-agent`; move remaining message/context assembly, compaction, evidence/provenance shaping, and intrinsic tests. Inject product guidance, memory, instructions, skills, and MCP summaries. |
...
| `src/agent/provider/` | `mez-agent` provider behavior plus root credential/HTTP/runtime adapters | temporary | Canonical request/response contracts, OpenAI Responses, compatible Chat Completions, Anthropic, DeepSeek, and Claude Code stable session-id policy now live in `mez-agent`. Root constant/response facades are removed; move remaining deterministic Claude Code prompt, schema, parsing, accounting, and retry policy. Retain concrete auth stores, HTTP/process transport, refresh, quota attachment, product response/error projection, and runtime event conversion in root adapters. |

...
| Mux presentation geometry and canvas primitives | `mez-mux` | `crates/mez-mux/src/{attached_client,presentation,render}` | temporary | Attached-client frame diffing and SGR/mode/cursor encoding, mouse and prefix boundaries, host-paste framing, generic wrapping, overlay cell geometry, color math, and style-span coalescing are lower-owned. Record-browser, overlay interaction, rich text, diff, frame, and prompt behavior remain scheduled by later phases. |
...
| `src/terminal/tests/` | product presentation and host integrations | temporary | Forty-one intrinsic attached-client tests now run in `mez-mux`; remaining temporary tests cover neutral frame and prompt rendering scheduled by Phase 9, while host and product-routing integrations stay root-owned. |

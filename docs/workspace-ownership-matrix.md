...
|---|---|---|---|
| `src/agent/actions/` | `mez-agent` harness; root local-action, permission, transcript, and runtime adapters | temporary | Initial action-result planning, approval transitions, auto-allow metadata, pending-runtime envelopes, structured local result shaping, memory guardrails, and default request gating are lower-owned. Root planning is a narrow adapter that supplies semantic plans, permission decisions, scope violations, and MCP approval facts. Move remaining deterministic recovery and turn execution state into `mez-agent`; retain concrete shell/MCP/filesystem dispatch and combined-product tests behind narrow ports. |

...
The decomposition is **complete**. The exhaustive machine-readable root
inventory is `docs/workspace-root-ownership.toml`; every root surface is now
classified as intentional product code or an adapter. This matrix records the
behavioral evidence behind those classifications. The architecture check
enforces dependency direction, exhaustive ownership, retired compatibility
surfaces, source-unit size, and the absence of `include!` flattening.
...
| Root surface | Final owner / root role | State | Boundary evidence |
|---|---|---|---|
| `src/terminal/render/` | product presentation adapters over `mez-mux` | adapter | Neutral frame plans, prompt-region placement, overlay composition, geometry, color math, and path fitting are mux-owned. Root retains decomposed product field resolution, readline/agent view-model construction, configured themes, animation, and hit actions. Lower forwarding aliases are removed, module-root imports are explicit, and every production unit is below the enforced size limit. |
...
| `src/terminal/tests/` | product presentation and host integrations | adapter | Intrinsic attached-client, frame-plan, prompt-layout/placement, overlay, and canvas tests run in `mez-mux`; root retains configured product-view and host-routing integrations. Retained tests are grouped into purpose-named modules, with no oversized or `include!`-flattened owner. |
...
5. The package audit proves five packages, exact dependency direction,
   exhaustive final root ownership, no lower-contract forwarding exports, no
   canonical root parser wrappers, and no wildcard imports in production
   agent/runtime/terminal module facades.
...
The decomposition acceptance criteria are satisfied. Future package-boundary
changes must update this matrix and the root ownership manifest while preserving
the dependency, ownership, compatibility, and source-structure guardrails.

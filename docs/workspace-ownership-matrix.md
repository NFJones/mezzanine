...
| `src/agent/context/` | `mez-agent` | temporary | Canonical capability, action-surface, context-source, model-message, `AgentContext`/`ContextBlock`, trust/cache metadata, compaction report, complete `ModelRequest`, and model-profile policy records are imported directly from `mez-agent`; move remaining message/context assembly, compaction algorithms, evidence/provenance shaping, and intrinsic tests. Inject product guidance, memory, instructions, skills, and MCP summaries. |

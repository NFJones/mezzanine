...
|---|---|---|---|
| `src/agent/actions/` | `mez-agent` harness; root local-action, permission, transcript, and runtime adapters | temporary | Structured shell-read observations plus bounded/pruned model-context and durable transcript action-result rendering are lower-owned with root forwarding removed. Move deterministic gating, planning, recovery, and turn execution state into `mez-agent`; retain concrete shell/MCP/filesystem dispatch behind narrow ports. |

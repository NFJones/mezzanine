...
| `src/agent/provider/` | `mez-agent` provider behavior plus root credential/HTTP/runtime adapters | temporary | Canonical contracts, OpenAI Responses and compatible Chat Completions behavior, plus Anthropic endpoint derivation, option parsing, message/system normalization, prompt-cache markers, effort/token controls, and MAAP request shaping now live in `crates/mez-agent/src/{openai_*,anthropic}.rs`. Move remaining provider-independent Anthropic response, DeepSeek, and model behavior. Retain concrete auth stores, HTTP metadata/transport, refresh, quota attachment, product response/error projection, and runtime event conversion in root adapters. |

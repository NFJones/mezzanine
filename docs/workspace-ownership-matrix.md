...
| Mux presentation geometry and canvas primitives | `mez-mux` | `crates/mez-mux/src/{presentation,render}.rs` | owned | Neutral canvas, divider, exact-width pane/window/group frame-row composition, generic frame/status template expansion, semantic right-status composition and placement, and Unicode-aware window/group frame pillbox layout are mux-owned. Product field resolution, palettes, animation, and hit-action policy remain adapters. |
...
| Root surface | Final owner / root role | State | Required migration |
|---|---|---|---|
| `src/terminal/render/` | `mez-mux` composition plus root product presentation adapter | temporary | Generic render cells, Unicode-width fitting/slicing, divider composition, exact-width pane/window/group frame-row layout, generic frame/status template parsing, semantic right-status segmentation and row placement, and window/group frame pillbox layout are mux-owned. Retain product field resolution, injected agent/prompt/permission/overlay view models, palettes, animation, and hit actions while moving the remaining neutral rendering behavior. |

...
|---|---|---|---|
| `src/agent/actions/` | `mez-agent` harness; root local-action, permission, transcript-store, and runtime adapters | temporary | Initial action-result and batch-result planning, approval transitions, auto-allow metadata, pending-runtime envelopes, structured local result shaping, memory guardrails, default request gating, ordered batch continuation, allowed-action validation, capability continuation, mixed/disallowed batch recovery, bounded MAAP repair requests, canonical `AgentTurnExecution`, deterministic execution transcript projection, and shell-output transport markers/base64 decoding/integrity diagnostics are lower-owned. Root planning supplies semantic plans, permission decisions, scope violations, and MCP approval facts; root recovery retains provider calls, retry integration, and product error projection; root transcript code appends through the concrete store; root execution retains pane/MCP dispatch. Move remaining deterministic turn execution state into `mez-agent`; retain concrete shell/MCP/filesystem dispatch and combined-product tests behind narrow ports. |
...
| `src/agent/tests/` | split by behavior owner | temporary | Readiness, context assembly, provider policy, semantic planning, execution-transcript projection, shell-output decoding, and 25 MAAP parsing/contract scenarios run directly in `mez-agent`. Root MAAP coverage is one concrete shell-policy callback integration; transcript, semantic, and shell coverage exercises product stores, pane execution, and generated wrappers; provider coverage exercises auth, HTTP/process transport, quota, retry, and product error adapters. Move remaining intrinsic harness tests and retain tests that exercise concrete product stores, transports, permissions, runtime, or UI. |
...
| `src/command/` | `mez-mux` mux command grammar/plans plus product dispatch | temporary | All typed session-mutation plans, command defaults, shell-argument reconstruction, and mux validation are lower-owned; root parsing only projects `MuxError` into `MezError`. Root retains session mutation, process lifecycle, product registry/status and outcomes, cross-cutting dispatch, and auth/config/audit/agent/store commands. Audit remaining session listing/chooser display shaping before reclassifying this tree as an adapter. |

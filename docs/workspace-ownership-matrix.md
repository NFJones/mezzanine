...
| Mux presentation geometry and canvas primitives | `mez-mux` | `crates/mez-mux/src/{presentation,render}.rs` | owned | Neutral canvas, divider, exact-width pane-frame row, and semantic right-status composition are mux-owned. Continue moving generic window/group frame composition while retaining product field, palette, animation, and hit-action policy in adapters. |
...
| Root surface | Final owner / root role | State | Required migration |
|---|---|---|---|
| `src/terminal/render/` | `mez-mux` composition plus root product presentation adapter | temporary | Generic render cells, Unicode-width fitting/slicing, divider composition, pane-frame row layout, and semantic right-status segmentation are mux-owned. Move remaining generic window/group frame composition; retain injected agent/prompt/permission/overlay view models, palettes, animation, and hit actions. |

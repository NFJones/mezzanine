...
| `src/agent/provider/` | `mez-agent` provider behavior plus root credential/HTTP/runtime adapters | temporary | Canonical contracts, OpenAI Responses and compatible Chat Completions behavior, plus Anthropic endpoint/request shaping, request-id-aware failure sanitization, and cache-aware token accounting now live in `crates/mez-agent/src/{openai_*,anthropic}.rs`. Move remaining provider-independent Anthropic content/stream response parsing, DeepSeek, and model behavior. Retain concrete auth stores, HTTP metadata/transport, refresh, quota attachment, product response/error projection, and runtime event conversion in root adapters. |

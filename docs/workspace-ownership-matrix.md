...
| `src/agent/context/` | `mez-agent` with root product-input adapters | adapter | Canonical records, compaction, request projection, evidence/cache/provider shaping, skill constraints, and all context appenders are lower-owned. Root context code supplies only turn identity and embedded prompt assets; production discovery, stores, registries, permissions, and runtime records feed lower APIs directly. All appender and request-shaping scenarios run lower; root retains one turn-record and embedded-asset adapter regression. |
...
| `src/agent/tests/` | split by behavior owner | temporary | Readiness, all 15 context-appender scenarios, all 10 request-shaping scenarios, and intrinsic OpenAI Responses and compatible Chat Completions request/response tests run directly in `mez-agent`; root context coverage is one product adapter regression. Move remaining intrinsic harness/provider/MAAP/patch tests and retain tests that exercise concrete product stores, transports, permissions, runtime, or UI. |

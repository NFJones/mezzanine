...
| `src/agent/provider/` | `mez-agent` provider behavior plus root credential/HTTP/runtime adapters | temporary | Canonical contracts plus OpenAI Responses request/cache/schema/response behavior and generic OpenAI-compatible Chat Completions option parsing, request shaping, envelope/MAAP parsing, finish-reason recovery, and token accounting now live in `crates/mez-agent/src/openai_*.rs`. Move remaining provider-independent Anthropic, DeepSeek, and model behavior. Retain concrete auth stores, reqwest/CLI transport, refresh, quota attachment, product response/error projection, and runtime event conversion in root adapters. |

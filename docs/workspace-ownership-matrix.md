...
| Mux presentation geometry and canvas primitives | `mez-mux` | `crates/mez-mux/src/{presentation,render}.rs` | owned | Move remaining neutral frame/divider/status composition and intrinsic tests from root rendering modules. |
...
| Root surface | Final owner / root role | State | Required migration |
|---|---|---|---|
| `src/terminal/render/` | `mez-mux` composition plus root product presentation adapter | temporary | Generic render cells, Unicode-width fitting/slicing, and style-span composition are mux-owned. Move remaining neutral frame/divider/status composition and intrinsic tests; retain injected agent/prompt/permission/overlay view models and product styles. |

...
| `src/agent/provider/` | `mez-agent` provider behavior plus root credential/HTTP/runtime adapters | temporary | Canonical contracts, OpenAI Responses and compatible Chat Completions behavior, complete Anthropic request/response behavior, and DeepSeek endpoint policy now live in `crates/mez-agent/src/{openai_*,anthropic,deepseek}.rs`. Move remaining provider-independent DeepSeek and model behavior. Retain concrete auth stores, HTTP metadata/transport, refresh, quota attachment, product response/error projection, and runtime event conversion in root adapters. |

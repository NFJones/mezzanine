...
| `src/agent/session.rs` | `mez-agent` session policy plus root product adapters | temporary | Canonical turn records, bounded ledger transitions, and all intrinsic ledger tests are lower-owned; the root turn module and facade are removed. Move agent-shell session/store state and retain only product display and identifier-generation adapters where necessary. |

...
| Agent contracts and provider-independent policy | `mez-agent` | `crates/mez-agent/src/` owns action surfaces, schemas, context policy, provider contracts, scheduler, canonical turn records/ledger, execution ports, production turn orchestration, patch parsing, shell transaction contracts, MMP delivery state, instruction planning, permission policy, subagent scope enforcement, and canonical memory, issue, transcript, and MCP domains | temporary | The canonical turn runner plus memory, issue, transcript, and MCP domains are lower-owned, but auto-sizing, completion recovery, progress ledgers, and shell-observation policy still require extraction from root. |

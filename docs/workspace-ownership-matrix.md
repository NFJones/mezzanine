...
| `src/agent/context/` | `mez-agent` with root product-input adapters | adapter | Canonical records, compaction, request projection, evidence/cache/provider shaping, skill constraints, and all context appenders are lower-owned. Root context code supplies only turn identity and embedded prompt assets; production discovery, stores, registries, permissions, and runtime records feed lower APIs directly. All 15 intrinsic appender scenarios run in purpose-named lower test modules; audit the remaining request-assembly integration cases for any lower-only coverage. |
...
| `src/agent/tests/` | split by behavior owner | temporary | Readiness, all 15 context-appender scenarios, and intrinsic OpenAI Responses and compatible Chat Completions request/response tests run directly in `mez-agent`; root context tests are reduced to 10 request-assembly integration scenarios. Move remaining intrinsic harness/provider/context/MAAP/patch tests and retain tests that exercise concrete product stores, transports, permissions, runtime, or UI. |

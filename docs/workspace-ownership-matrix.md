...
| Agent contracts and provider-independent policy | `mez-agent` | `crates/mez-agent/src/` owns action surfaces, schemas, context policy, provider contracts, scheduler, canonical turn records/ledger, execution ports, production turn orchestration, patch parsing, shell transaction contracts, MMP delivery state, instruction planning, permission policy, subagent scope enforcement, and canonical memory and issue domains | temporary | The canonical turn runner, memory domain, and issue domain are lower-owned, but MCP protocol/registry state, transcript contracts, auto-sizing, completion recovery, progress ledgers, and shell-observation policy still require extraction from root. |
...
| MCP and transcript domains | `mez-agent` domain plus root persistence/transport adapters | temporary | Portable protocol state, records, validation, and policy still remain mixed with concrete root adapters. |
...
| `src/transcript/` | agent domain plus concrete repository | temporary | Canonical records and deterministic summary policy still need to move lower; root keeps filesystem storage and migration. |

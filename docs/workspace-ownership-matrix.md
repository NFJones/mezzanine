...
| MCP, memory, issues, permissions, and transcript domains | `mez-agent` domain plus root persistence/transport adapters | temporary | Portable protocol state, records, validation, and policy still remain mixed with concrete root adapters. |
...
| `src/mcp/`, `src/permissions/`, `src/subagent/` | agent policy/protocol plus root product adapters | temporary | Portable policy, plans, state, or enforcement still require extraction. Instruction discovery has moved completely to `mez-agent` and no longer has a root surface. |

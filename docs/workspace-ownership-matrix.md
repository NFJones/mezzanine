...
| `src/command/` | `mez-mux` pure mux grammar/plans plus product dispatch | temporary | Canonical command invocation records, quoting/tokenization, semicolon sequencing, and basic flag/positional queries are lower-owned; root parsing only projects `MuxError` into `MezError`. Move pure mux mutation plans next. Keep product registry/status, cross-cutting dispatch, auth/config/audit/agent/store commands, display policy, and product outcomes root-owned. |

...
| `src/agent/provider/` | `mez-agent` provider behavior plus root credential/HTTP/runtime adapters | temporary | Canonical request and response contracts plus OpenAI Responses, compatible Chat Completions, Anthropic, and DeepSeek provider-independent request/response behavior now live in `mez-agent`. Root constant/response facades are removed; audit remaining adapter exports and deterministic provider behavior. Retain concrete auth stores, HTTP metadata/transport, refresh, quota attachment, product response/error projection, and runtime event conversion in root adapters. |

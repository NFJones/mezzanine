...
|---|---|---|---|
| `src/agent/actions/` | `mez-agent` harness; root local-action, permission, transcript, and runtime adapters | temporary | Structured shell-read observations, action-result context rendering, per-turn memory guardrails, and default MCP/memory/issue request gating are lower-owned with root forwarding removed; their intrinsic policy regressions run lower. Move remaining deterministic planning, recovery, and turn execution state into `mez-agent`; retain concrete shell/MCP/filesystem dispatch and combined-product tests behind narrow ports. |

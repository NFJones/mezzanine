...
| `src/agent/provider/` | `mez-agent` provider behavior plus root credential/HTTP/runtime adapters | temporary | Canonical contracts plus OpenAI Responses, compatible Chat Completions, Anthropic, and DeepSeek provider-independent request/response behavior now live in `crates/mez-agent/src/{openai_*,anthropic,deepseek*}.rs`. Move remaining provider-independent model behavior. Retain concrete auth stores, HTTP metadata/transport, refresh, quota attachment, product response/error projection, and runtime event conversion in root adapters. |

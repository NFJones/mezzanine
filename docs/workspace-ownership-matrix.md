...
| `src/agent/context/` | `mez-agent` with root product-input adapters | adapter | Canonical records, compaction, request projection, evidence/cache/provider shaping, skill constraints, memory selection, explicit MCP filtering and manifest shaping, repository-guidance wrapping and placement, permission-context omission, and scheduler-state projection are lower-owned. Root context code supplies only turn identity and embedded prompt assets; production discovery, stores, registries, permissions, and runtime records feed lower appenders directly. Relocate remaining intrinsic appender tests while retaining product-input and prompt-assembly integration coverage in root. |

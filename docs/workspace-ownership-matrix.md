...
| `src/agent/provider/` | `mez-agent` provider behavior plus root credential/HTTP/runtime adapters | temporary | Canonical contracts plus OpenAI Responses request rendering/body construction, stable schema/tool shaping, prompt-cache diagnostics, JSON/SSE parsing, native MAAP argument accumulation, and token accounting now live in `crates/mez-agent/src/openai_{cache,request,response,schema}.rs`. Move remaining provider-independent OpenAI-compatible/Anthropic/DeepSeek request, response, and model behavior. Retain concrete auth stores, reqwest/CLI transport, refresh, product error projection, and runtime event conversion in root adapters. |
...
| `src/agent/tests/` | split by behavior owner | temporary | Readiness plus intrinsic OpenAI request-rendering and response-parser tests now run directly in `mez-agent`; root OpenAI request/cache tests exercise product context assembly against lower request shaping. Move remaining intrinsic harness/provider/context/MAAP/patch tests and retain tests that exercise concrete product stores, transports, permissions, runtime, or UI. |

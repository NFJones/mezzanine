...
| Mux presentation geometry and canvas primitives | `mez-mux` | `crates/mez-mux/src/{presentation,render}.rs` | owned | Neutral pane-to-canvas composition, divider rendering, exact-width pane/window/group frame-row composition, generic frame/status template expansion, semantic right-status composition and placement, and Unicode-aware window/group frame pillbox layout are mux-owned. Product field resolution, merged-frame overlays, palettes, animation, and hit-action policy remain adapters. |
...
| Root surface | Final owner / root role | State | Required migration |
|---|---|---|---|
| `src/terminal/render/` | `mez-mux` composition plus root product presentation adapter | temporary | Generic render cells, pane placement and clipping, plain/styled pane canvas composition, divider composition, exact-width pane/window/group frame-row layout, generic frame/status template parsing, semantic right-status segmentation and row placement, window/group frame pillbox layout, terminal color/contrast math, animation gradients, and style-span coalescing are mux-owned. Retain product field resolution, merged-frame overlays, injected agent/prompt/permission/overlay view models, palettes, animation timing, and hit actions while moving the remaining neutral rendering behavior. |

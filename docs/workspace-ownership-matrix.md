...
| `src/agent/provider/` | `mez-agent` provider behavior plus root credential/HTTP/runtime adapters | temporary | Canonical contracts, OpenAI Responses and compatible Chat Completions behavior, plus complete Anthropic request/response behavior now live in `crates/mez-agent/src/{openai_*,anthropic}.rs`. Move remaining provider-independent DeepSeek and model behavior. Retain concrete auth stores, HTTP metadata/transport, refresh, quota attachment, product response/error projection, and runtime event conversion in root adapters. |

...
|---|---|---|---|
| `src/agent/actions/` | `mez-agent` harness plus root local-action, permission, provider, transcript-store, and runtime adapters | adapter | Initial and resumed action-result planning, batch continuation/recovery, memory guardrails, request gating, canonical execution projection, lifecycle derivation, transcript projection, and shell-output decoding are lower-owned. Root planning supplies semantic plans, concrete permission/scope/MCP facts, provider calls and failure-summary I/O; root transcript code appends through the concrete store; root execution retains pane/MCP/filesystem dispatch and product error projection behind lower contracts. |

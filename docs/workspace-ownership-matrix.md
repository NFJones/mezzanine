...
| Agent contracts and provider-independent policy | `mez-agent` | `crates/mez-agent/src/` owns action surfaces, schemas, context policy, provider contracts, scheduler, canonical turn records/ledger, execution ports, production turn orchestration, patch parsing, shell transaction contracts, MMP delivery state, instruction planning, permission policy, subagent scope enforcement, and canonical memory records/session state | temporary | The canonical turn runner and memory domain are lower-owned, but MCP protocol/registry state, issue/transcript contracts, auto-sizing, completion recovery, progress ledgers, and shell-observation policy still require extraction from root. |
...
| MCP, issues, and transcript domains | `mez-agent` domain plus root persistence/transport adapters | temporary | Portable protocol state, records, validation, and policy still remain mixed with concrete root adapters. |
...
| `src/mcp/` | agent protocol plus root product transports | temporary | Portable protocol and registry state still require extraction while process, HTTP, credential, and audit adapters remain root-owned. |
...
| `src/issues/`, `src/transcript/` | agent domains plus concrete repositories | temporary | Canonical records and deterministic policy still need to move lower; root keeps SQLite/filesystem storage and migration. |

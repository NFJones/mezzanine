...
| `src/command/` | `mez-mux` pure mux grammar/plans plus product dispatch | temporary | Canonical invocation grammar plus resize, swap, break, join, and split-selection plans are lower-owned; root parsing only projects `MuxError` into `MezError`. Root retains mixed command classification, process-spawn shell/display policy, product registry/status and outcomes, cross-cutting dispatch, and auth/config/audit/agent/store commands. Audit the remaining neutral group/window/pane plans next. |

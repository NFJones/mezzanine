...
| Root surface | Final owner / root role | State | Required migration |
|---|---|---|---|
| `src/terminal/render/` | `mez-mux` composition plus root product presentation adapter | temporary | Generic render cells, Unicode-width fitting/slicing, divider composition, pane-frame row layout, semantic right-status segmentation, and window/group frame pillbox layout are mux-owned. Move remaining generic frame-row composition; retain injected agent/prompt/permission/overlay view models, palettes, animation, and hit actions. |

...
|---|---|---|---|
| `src/agent/actions/` | `mez-agent` harness; root local-action, permission, transcript, and runtime adapters | temporary | Structured shell-read observations, action-result context rendering, and per-turn memory budgeting/wrapper-placeholder guardrails are lower-owned with root forwarding removed; intrinsic parser, renderer, transcript-summary, status-invariant, and memory-guardrail regressions run lower. Move remaining deterministic gating, planning, recovery, and turn execution state into `mez-agent`; retain concrete shell/MCP/filesystem dispatch and combined-product tests behind narrow ports. |

...
| `src/agent/tests/` | split by behavior owner | temporary | Readiness, context appenders, request shaping, provider schema/capability/compatibility/accounting policy, intrinsic OpenAI Responses and compatible Chat Completions request/response behavior, and semantic-patch parser/planner/transaction scenarios run directly in `mez-agent`. Root semantic-patch coverage is one combined pane-executor/action-result integration; root provider coverage exercises auth, HTTP/process transport, quota, retry, and product error adapters. Move remaining intrinsic harness/MAAP tests and retain tests that exercise concrete product stores, transports, permissions, runtime, or UI. |

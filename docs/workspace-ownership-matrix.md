...
| `src/agent/provider/` | `mez-agent` provider behavior plus root credential/HTTP/runtime adapters | temporary | Canonical contracts, OpenAI Responses and compatible Chat Completions behavior, plus Anthropic endpoint/request shaping, failure sanitization, token accounting, content/MAAP interpretation, and stop-reason recovery now live in `crates/mez-agent/src/{openai_*,anthropic}.rs`. Move remaining provider-independent Anthropic JSON/SSE parsing, DeepSeek, and model behavior. Retain concrete auth stores, HTTP metadata/transport, refresh, quota attachment, product response/error projection, and runtime event conversion in root adapters. |
...
| `src/agent/tests/` | split by behavior owner | temporary | Readiness plus intrinsic OpenAI Responses and compatible Chat Completions request/response tests now run directly in `mez-agent`; root request/cache/provider tests exercise product context, transport, quota, response, and error integration against lower provider behavior. Move remaining intrinsic harness/provider/context/MAAP/patch tests and retain tests that exercise concrete product stores, transports, permissions, runtime, or UI. |

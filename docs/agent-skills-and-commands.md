...
`/show-issues` and `/show-memories` use the shared command-output pager. `/` keeps the normal in-page text search, while record-browser keys provide database-backed actions: `k` opens the kind dropdown selector, `p` opens the project/scope filter, `x` opens the full-text filter, `s` opens the save prompt, `Enter` opens the focused record, and `Esc` closes prompts, returns from detail to list, or exits the list view.

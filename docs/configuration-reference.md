...
The current config schema version is `50`. On launch, Mezzanine migrates an
...
| --- | --- | --- | --- |
| `version` | integer | `50` | Config schema version. Do not change this. |
...
| `permissions.bubblewrap.env_whitelist` | string array | `[]` | Schema-v50 primary-user-only portable variable names read from the active pane process. Values are best-effort, bounded, universally redacted from status/logs, and cannot override fixed sandbox environment invariants. |

...
| `terminal.term` | string | `"screen-256color"` | `TERM` value exposed to newly launched panes; must not claim host identity such as `xterm-256color`. For `screen` and `screen-*`, presentation maps italic to inverse video because Screen terminfo encodes standout (including `less` search matches) as italic. Other profiles preserve true italics. Existing panes retain the policy selected when they launched. |

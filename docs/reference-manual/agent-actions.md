...
| `close_agent` | Retire one caller-owned persistent child. | The runtime authorizes only the current parent conversation's live persistent child using authoritative ownership metadata. Discover targets through `list_agents`; inaccessible and stale targets intentionally return the same opaque unavailable result. |

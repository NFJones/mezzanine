...
/// Default pane-facing TERM for the xterm-compatible terminal profile.
pub const DEFAULT_PANE_TERM: &str = "xterm-256color";
...
impl TerminalProfileError {
    /// Returns the diagnostic message for this profile lookup failure.
    pub fn message(&self) -> &str {
        &self.message
    }
}

impl Display for TerminalProfileError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        formatter.write_str(&self.message)
    }
}

impl Error for TerminalProfileError {}
...
pub fn terminal_profile_named(
    name: &str,
) -> Result<TerminalCompatibilityProfile, TerminalProfileError> {
    match name {
        ...
        _ => Err(TerminalProfileError {
            message: format!("unknown terminal compatibility profile {name}"),
        }),
    }
}

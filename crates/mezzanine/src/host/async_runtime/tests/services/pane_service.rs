...
async fn async_unmanaged_bash_first_prompt_completes_deferred_bootstrap() {
    ...
    assert!(
        ...
        "pane mode must publish authority after successful dependency-free bootstrap"
    );
    actor_exit.service.terminate_all_pane_processes().unwrap();
}
...
async fn async_unmanaged_primary_zsh_bootstrap_executes_post_bootstrap_input() {
    ...
    let client = async move {
        ...
        assert!(
            last_snapshot.environment_signature_present,
            "successful pane-mode certification must publish environment authority: {last_snapshot:?}"
        );
    ...
    assert!(
        !actor_exit
            .service
            .pane_environment_authority_is_certified_for_tests("%1"),
        "hiding agent mode must invalidate the exited child environment authority"
    );
    actor_exit.service.terminate_all_pane_processes().unwrap();

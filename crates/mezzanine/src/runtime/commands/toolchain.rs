        ...
        let (
            ...
            Some(signature) => {
                ...
                let dotnet_root = match resolve_toolchain_projection(
                    ...
                    Ok(Some(projection)) => {
                        discoverable.push("dotnet".to_string());
                        projection
                            .roots
                            .first()
                            .map(|root| root.host_path.display().to_string())
                    ...
                    Ok(None) => None,
                    Err(error) => {
                        errors.push(format!("dotnet:{}", error.message()));
                        None
                    }
                };
    ...
    match kind {
        ...
        SandboxToolchainKind::Dotnet => {
            let projection = resolve_toolchain_projection(&[kind], environment_managers, host_os)
                .map_err(|error| MezError::invalid_state(error.message()))?
                .ok_or_else(|| {
                    MezError::invalid_state(".NET projection unexpectedly resolved empty")
                })?;
            let root = projection.roots.first().ok_or_else(|| {
                MezError::invalid_state(".NET projection is missing its SDK root")
            })?;
            Ok(format!(
                "dotnet_root={} sandbox_path={}",
                json_escape(&root.host_path.display().to_string()),
                SANDBOX_DOTNET_PATH,
            ))
        }
...
fn toolchain_status_host_evidence(kind: SandboxToolchainKind, status: &ToolchainStatus) -> String {
    let values = match kind {
        ...
        SandboxToolchainKind::Gradle => vec![status.gradle_root.as_deref()]
            ...
            .flatten()
            .collect(),
        SandboxToolchainKind::Dotnet => vec![status.dotnet_root.as_deref()]
            .into_iter()
            .flatten()
            .collect(),
...
mod tests {
    ...
    fn toolchain_parser_is_strict_and_requires_confirmation() {
        ...
        assert_eq!(
            parse_toolchain_command("detect dotnet").unwrap(),
            ToolchainCommand::Detect(ToolchainSelection::BuiltIn(SandboxToolchainKind::Dotnet,))
        );
        assert_eq!(

...
Work one returned issue at a time, choosing an issue whose `depends_on` list is empty or already resolved. Prioritize dependency-free in-progress issues over open issues; choose an open issue only when no eligible in-progress issue remains. In the first action batch after choosing, name the selected issue id in the batch rationale and record `Active issue: <id>` plus the durable implementation direction in `thought`. Keep using that selected id until the issue is resolved or explicitly blocked. Before implementing, inspect the cited code, tests, docs, and spec enough to form a concrete execution plan for that issue. Do not issue another backlog query while the selected issue is being inspected, implemented, documented, or validated. If all remaining issues are blocked by dependencies that are absent from the query results, use a narrowly filtered query or inspect the missing issue ids before proceeding; report a blocker if the dependency graph cannot be resolved.

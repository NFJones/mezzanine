...
"#;
    ...
    assert!(plan.changed);
    assert!(plan.text.contains("version = 48"));
...
"#;
    ...
    assert_eq!(plan.to_version, CURRENT_CONFIG_SCHEMA_VERSION);
    assert!(plan.changed);
    assert_eq!(values.get("version"), Some(&"48".to_string()));
    assert_eq!(
...
fn migrates_terminal_emoji_width_default_to_current_schema() {
    ...
    assert_eq!(missing_values.get("version"), Some(&"48".to_string()));
    assert_eq!(
    ...
    assert_eq!(explicit_values.get("version"), Some(&"48".to_string()));
    assert_eq!(
...
fn migrates_schema_25_without_enabling_zig() {
    for input in [
        ...
        assert_eq!(values.get("version"), Some(&"48".to_string()));
        assert_eq!(
            values.get("permissions.bubblewrap.toolchains"),
            input
                .contains("toolchains")
                .then(|| "[\"rust\"]".to_string())
                .as_ref()
...
fn migrates_schema_28_without_enabling_bun() {
    for input in [
        ...
        assert_eq!(
            ...
            input
                .contains("toolchains")
                .then(|| "[\"rust\", \"zig\", \"go\", \"deno\"]".to_string())
                .as_ref()
        );
...
fn migrates_schema_29_without_enabling_node() {
    for input in [
        ...
        assert_eq!(
            ...
            input
                .contains("toolchains")
                .then(|| "[\"rust\", \"zig\", \"go\", \"deno\", \"bun\"]".to_string())
                .as_ref()
        );
...
fn migrates_schema_30_without_enabling_python() {
    for input in [
        ...
        assert_eq!(
            ...
            input
                .contains("toolchains")
                .then(|| { "[\"rust\", \"zig\", \"go\", \"deno\", \"bun\", \"node\"]".to_string() })
                .as_ref()
        );
...
fn migrates_schema_32_without_enabling_jdk() {
    for input in [
        ...
        assert_eq!(
            ...
            input
                .contains("toolchains")
                .then(|| "[\"rust\", \"custom:acme\"]".to_string())
                .as_ref()
        );
...
fn migrates_schema_33_without_enabling_dotnet() {
    for input in [
        ...
        assert_eq!(
            ...
            input
                .contains("toolchains")
                .then(|| "[\"jdk\", \"custom:acme\"]".to_string())
                .as_ref()
        );
...
fn migrates_schema_34_without_enabling_dart() {
    for input in [
        ...
        assert_eq!(
            ...
            input
                .contains("toolchains")
                .then(|| "[\"dotnet\", \"custom:acme\"]".to_string())
                .as_ref()
        );
...
fn migrates_schema_35_without_enabling_kotlin() {
    for input in [
        ...
        assert_eq!(
            ...
            input
                .contains("toolchains")
                .then(|| "[\"jdk\", \"dart\", \"custom:acme\"]".to_string())
                .as_ref()
        );
...
fn migrates_schema_36_without_enabling_ruby() {
    for input in [
        ...
        assert_eq!(
            ...
            input
                .contains("toolchains")
                .then(|| "[\"kotlin\", \"custom:acme\"]".to_string())
                .as_ref()
        );
...
fn migrates_schema_37_without_enabling_php_or_composer() {
    for input in [
        ...
        assert_eq!(
            ...
            input
                .contains("toolchains")
                .then(|| "[\"ruby\", \"custom:acme\"]".to_string())
                .as_ref()
        );
...
fn migrates_schema_38_without_enabling_erlang_or_elixir() {
    for input in [
        ...
        assert_eq!(
            ...
            input
                .contains("toolchains")
                .then(|| "[\"composer\", \"custom:acme\"]".to_string())
                .as_ref()
        );
...
fn migrates_schema_39_without_enabling_haskell_toolchains() {
    for input in [
        ...
        assert_eq!(
            ...
            input
                .contains("toolchains")
                .then(|| "[\"elixir\", \"custom:acme\"]".to_string())
                .as_ref()
        );
...
fn migrates_schema_40_without_enabling_ocaml() {
    for input in [
        ...
        assert_eq!(
            ...
            input
                .contains("toolchains")
                .then(|| "[\"stack\", \"custom:acme\"]".to_string())
                .as_ref()
        );
...
fn migrates_schema_41_without_enabling_native_toolchains() {
    for input in [
        ...
        assert_eq!(
            ...
            input
                .contains("toolchains")
                .then(|| "[\"ocaml\", \"custom:acme\"]".to_string())
                .as_ref()
        );
...
fn migrates_schema_42_without_enabling_swift() {
    for input in [
        ...
        assert_eq!(
            ...
            input
                .contains("toolchains")
                .then(|| "[\"llvm\", \"custom:acme\"]".to_string())
                .as_ref()
        );
...
#[test]
fn migrates_schema_47_with_empty_supplementary_groups() {
    for (format, input) in [
        (
            ConfigFormat::Toml,
            "version = 47\n[permissions]\nsandbox = \"bubblewrap\"\n",
        ...
        assert_eq!(values.get("version"), Some(&"48".to_string()));
        assert_eq!(
            values.get("permissions.bubblewrap.supplementary_groups"),
            Some(&"[]".to_string())
        );
    }
}

...
# Opt in to low-latency history reuse with "lz4-stream" or "zstd-stream".
compression_min_bytes = 512
compression_zstd_level = 3

# X11 forwarding is separately opt-in and applies only to authenticated Iroh
# primary attachments. Changes require a daemon restart.
[transport.iroh.x11]
enabled = false
allow_trusted = false
...
# Server commands use the session X11 proxy environment when enabled.
# Negotiated Iroh copies instead use the attaching client's local configuration.
...
zen_mode = false
# Transient zen focus-label lifetime; zero disables labels (maximum 60000).
zen_focus_label_duration_ms = 1000
...
# Coalesce terminal resize redraws and the one full redraw after releasing a
# changed pane-divider drag for this many milliseconds.
...
# Presets supply both rails. Uncomment either line to override that rail;
# an explicitly empty string intentionally keeps that rail empty.
#? left_status = "#{pane.progress}"
#? right_status = "#{pane.pwd} #{agent.model} #{agent.reasoning} #{agent.thinking} #{agent.planning} #{agent.routing} #{agent.latency} #{policy.mode} #{agent.context_usage} #{agent.status} #{history.position}"
overflow = "menu"
title_min_width = 8
style = "default"
...
[frames.pane.pills]
# Optional named built-in or command-backed pill referenced as #{pill.<name>}.
...
# Pane command providers require trusted config, cwd = "pane", an explicit
# permission Allow decision, and a working OS sandbox. They receive only the
# documented MEZ_PANE_ID binding; missing authority leaves the pill inert.
...
# Custom actions require trusted exact-source provenance. Supported terminal
# actions are rename-pane, copy-mode, and copy-selection with one -t {pane};
# supported agent actions are /plan and /stop.
...
surface = "#1b1f0a"
container = "#2f302b"
container_foreground = "#eef7d0"
container_muted_foreground = "#8e9e5b"
container_primary_foreground = "#bfff00"
container_secondary_foreground = "#7fbf3f"
container_tertiary_foreground = "#d7ff5f"
container_danger_foreground = "#ff5c57"
danger_foreground = "#ff5c57"
danger_text = "#000000"
muted_text = "#000000"
primary_foreground = "#bfff00"
primary_text = "#000000"
secondary_foreground = "#7fbf3f"
secondary_text = "#000000"
tertiary_foreground = "#d7ff5f"
...
rotate_lines = 1000
# Maximum active saved agent conversations retained on disk.
saved_sessions_limit = 10000
# Maximum age in days since the last durable activity of an active saved conversation.
saved_sessions_retention_days = 90
...
# After adding a compatible provider, use `mez config model add PROVIDER MODEL_ID`
# to create validated model records without hand-writing generated entry keys.
...
# wakefulness where supported. Native Linux uses systemd-logind plus the desktop
# ScreenSaver D-Bus service; macOS uses IOKit. Detached sessions retain the
# request while work runs. Backends are best-effort and never override explicit
# sleep, lid-close, thermal, or critical-battery safeguards.
...
enabled_actions = ["say", "shell_command", "apply_patch", "web_search", "fetch_url", "send_message", "wait", "spawn_agent", "close_agent", "config_change", "mcp_server_search", "mcp_server_get", "mcp_call", "memory_search", "memory_store", "list_agents", "issue_add", "issue_update", "issue_query", "issue_delete"]
...
# Default maximum duration for one native shell `shell_command` action.
# The effective timeout is the earliest of this value, an explicit action
# timeout, and the remaining turn budget. Pane-shell actions do not use it.
native_shell_timeout_ms = 600000
...
# Pane echo of peer MMP traffic: `normal` logs canonical
# `text/plain; charset=utf-8` and supported `text/markdown` payloads; JSON,
# binary, and absent media types remain durable and model-visible without pane
# rows. Accepted eligible model sends may add one `${recipient}<` sender row,
# while bridge traffic remains receiver-only. `verbose` adds bounded raw media.
peer_message_log_mode = "normal"
# Saved-session title source: `generated` stores one bounded model-authored
# title per conversation, else falls back to the agent objective and then the
# first prompt; `objective`, `last_prompt`, and `first_prompt` mirror that source
# without a provider call. A manual `/name-session` name always wins.
session_title_policy = "generated"
# Optional cheaper model profile for generated session titles; empty uses the
# conversation's own model profile, and an unresolvable name also falls back to
# it.

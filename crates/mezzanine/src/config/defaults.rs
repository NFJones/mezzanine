...
# Opt-in read-only Rust, Zig, Go, Deno, Bun, Node.js, or Python projections;
# detect with `--kind rust|zig|go|deno|bun|node|python`, then
...
# toolchains = ["rust", "zig", "go", "deno", "bun", "node", "python"]
# Custom toolchains are primary-config-only constrained definitions. They are
# disabled by default and portable sandbox profiles reject them to avoid
# serializing host roots. Runtime projection also requires existing pane read
# authority; definitions never grant authority.

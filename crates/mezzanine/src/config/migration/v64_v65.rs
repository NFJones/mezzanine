//! Configuration schema v64 to v65 migration.
//!
//! Schema v65 makes the total agent-turn wall-clock budget configurable.
//! Existing primary configurations receive the historical 30-minute value.
...
/// Adds the agent-turn timeout default and advances the schema version.
pub(super) fn migrate_v64_to_v65(format: ConfigFormat, text: &str) -> Result<String> {
    match format {
        ConfigFormat::Toml => {
            ...
            copy_toml_default_if_absent(&mut document, &defaults, "agents.turn_timeout_ms")?;
            set_toml_path_item(&mut document, "version", toml_edit::value(65))?;
        ...
        ConfigFormat::Yaml | ConfigFormat::Json => {
            ...
            copy_json_default_if_absent(&mut document, &defaults, "agents.turn_timeout_ms")?;
            set_json_path_value(&mut document, "version", serde_json::json!(65))?;
            match format {

//! Configuration schema v38 to v39 migration.
//!
//! Schema v39 adds the code-owned Erlang/OTP runtime and Elixir companion
...
/// Advances a schema v38 configuration document to v39 without changing data.
pub(super) fn migrate_v38_to_v39(format: ConfigFormat, text: &str) -> Result<String> {
    match format {
        ConfigFormat::Toml => {
            let mut document = text
                .parse::<toml_edit::DocumentMut>()
                .map_err(|error| MezError::config(format!("invalid TOML config: {error}")))?;
            set_toml_path_item(&mut document, "version", toml_edit::value(39))?;
            Ok(document.to_string())
        }
        ConfigFormat::Yaml | ConfigFormat::Json => {
            let mut document = parse_json_compatible_config(format, text)?;
            set_json_path_value(&mut document, "version", serde_json::json!(39))?;
            match format {

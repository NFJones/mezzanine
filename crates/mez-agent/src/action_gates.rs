...
use crate::{McpPromptTool, ModelRequest};
...
mod tests {
    ...
    #[test]
    /// Verifies integration availability is recorded without changing the
    /// configured static action surface.
    fn default_action_gates_expose_mcp_and_memory_for_diagnostic_request_shapes() {
        ...
        let configured = request.allowed_actions.clone();
        let tools = vec![mcp_tool()];

        apply_default_action_gates(&mut request, &tools, true, false);

        assert_eq!(request.allowed_actions, configured);
    ...
    #[test]
    /// Verifies unavailable integrations remain runtime validation concerns
    /// rather than mutating the provider-visible action set.
    fn default_action_gates_preserve_actions_for_unavailable_integrations() {
        let mut request = request();
        request.allowed_actions = AllowedActionSet::all_enabled();
        let configured = request.allowed_actions.clone();

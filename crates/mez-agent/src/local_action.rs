...
/// Declares which interpreter grammar owns one generated local program.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum LocalProgramDialect {
    /// Execute the source with the active pane shell.
    PaneNative,
    /// Execute the source with the portable POSIX shell interpreter.
    PosixSh,
}

impl LocalProgramDialect {
    /// Returns an explicit interpreter path when the program is not pane-native.
    pub const fn interpreter_path(self) -> Option<&'static str> {
        match self {
            Self::PaneNative => None,
            Self::PosixSh => Some("/bin/sh"),
        }
    }
}
...
pub struct LocalActionPlan {
    ...
    /// Interpreter grammar required by the generated command source.
    pub program_dialect: LocalProgramDialect,
    /// Concise user-facing summary shown before the action executes.
    pub summary: String,
    /// Exact pane shell input generated by the harness.
    pub command: String,
    /// Optional framed data consumed separately from generated shell source.
    ///
    /// Semantic writes use this channel to keep large file bytes out of the
    /// command program while retaining pane-shell execution and authorization.
    pub input_sidecar: Option<String>,
    ...
    match &action.payload {
        AgentActionPayload::ShellCommand {
            ...
            Ok(Some(LocalActionPlan {
                ...
                program_dialect: LocalProgramDialect::PaneNative,
...
mod tests {
    ...
    fn local_action_plan_preserves_execution_contract() {
        ...
        assert_eq!(plan.program_dialect, LocalProgramDialect::PaneNative);

...
/// Returns the opaque terminal bytes emitted after an agent subshell exits.
///
/// The marker is generated by the runtime for one pane handoff and written by
...
fn agent_subshell_exit_marker_command(marker: &MarkerToken) -> String {
    format!(
...
/// Renders an agent-mode subshell handoff with an optional parent-owned exit boundary.
///
/// When supplied, `exit_marker` is emitted by the parent shell only after the
...
pub fn agent_subshell_enter_command_with_shell_compatibility_and_exit_marker(
    ...
    let direct_source_exit_boundary = exit_marker
        .map(agent_subshell_exit_marker_command)
        .unwrap_or_default();
    let source = if classification == ShellClassification::Fish {
...
command env \\
  ...
  {shell_invocation}
{history_restore}
{direct_source_exit_boundary}\
end
",
            history_start = fish_shell_history_suppression_start(),
            history_restore = fish_shell_history_restore(),
            direct_source_exit_boundary = direct_source_exit_boundary,
            env_words = env_words,

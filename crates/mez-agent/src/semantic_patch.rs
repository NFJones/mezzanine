...
impl fmt::Display for SemanticPatchParseError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(&self.message)
    }
}

impl std::error::Error for SemanticPatchParseError {}

fn apply_patch_parse_error<T>(message: &str) -> SemanticPatchResult<T> {
    Err(SemanticPatchParseError::new(message))
}
...
pub struct MezPatch {
    ...
    pub operations: Vec<MezPatchOperation>,
}
...
pub fn parse_mez_patch(text: &str) -> SemanticPatchResult<MezPatch> {
...
) -> SemanticPatchResult<(usize, MezPatchHunk, Option<bool>)> {
    if index >= lines.len() {

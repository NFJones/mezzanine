...
Mezzanine is a Unix terminal muxxer for agentic development. It builds the
`mez` binary and combines tmux-style sessions, windows, panes, and copy mode
with a pane-scoped AI agent harness.

The normative behavior is defined in `SPEC.md`. This README is a user-oriented
guide to the current repository and generated default configuration.
...
- Agent work is pane-scoped. Normal pane input, other panes, and muxxer keys
...
### 4. Work in the muxxer

Default workflow keys follow tmux-style placement:
...
non-modal: other panes and muxxer navigation remain available.
...
| `frames.window.right_status` | string | `"#{pane.pwd} #{button:-|terminal|split-window -h} #{button:+|terminal|split-window} #{button:□|terminal|new-window} #{button:⊕|terminal|new-group} #{button:λ|terminal|agent-shell} #{system.uptime} #{datetime.local}"` | Right-aligned status and command buttons. |

...
- `mez sandbox toolchains list`, `status [SELECTOR] [PATH]`, and
  `detect [--kind rust|zig|go|deno|bun|node|python|jdk|dotnet|dart|kotlin|ruby|php|composer|erlang|elixir] [PATH]` inspect effective
  ...
  Composer, Erlang/OTP, and Elixir mount one
...
- In the agent shell, `/toolchain` and `/toolchain status` report supported,
  ...
  `/toolchain detect [rust|zig|go|deno|bun|node|python|jdk|dotnet|dart|kotlin|ruby|php|composer|erlang|elixir]` is read-only and uses only
